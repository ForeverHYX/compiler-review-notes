...
- 定位题：给一个 bug 现象，能定位到 lexer/parser/semantic/frame/translate/codegen/liveness/regalloc/GC。
- 串联题：能把一个小 Tiger 片段从 token、AST、semantic、IR、assembly、liveness 到 regalloc 串起来。
- 英文题：看到 `procedure fragment`、`string fragment`、`procEntryExit`、`runtime library` 能说出职责。
...
| 阶段 | 可能结果 |
|---|---|
| Lexer | `LET VAR ID(x) ASSIGN NUM(1) VAR ID(y) ASSIGN NUM(2) IN ID(x) ASSIGN ID(x) PLUS ID(y) END` |
| Parser/AST | `LetExp([VarDec(x,1), VarDec(y,2)], AssignExp(x, OpExp(PLUS,x,y)))` |
| Semantic | `x:int`、`y:int`，赋值左右类型兼容，`let` body 为 `unit` |
| Frame | 若 `x/y` escape=false，可为 `InReg(tx)`、`InReg(ty)`；否则为 `InFrame(offset)` |
| IR | `MOVE(tx,1); MOVE(ty,2); MOVE(tx, BINOP(PLUS,tx,ty))` |
| Canon | 已是线性语句，无 `ESEQ/CALL` |
| Basic block | 一个以 `LABEL` 开头、以 `JUMP done` 结尾的块 |
| Codegen | `li tx,1; li ty,2; add tx,tx,ty` |
| Liveness | `ty` 在 add 前活跃；旧 `tx` 在 add 前活跃，add 后新 `tx` 若无使用则不活跃 |
| RegAlloc | `tx`、`ty` 若不干涉或干涉少，可分配到物理寄存器 |

这个例子展示了一个重要事实：前端的类型信息、frame access、IR temporary 会一路影响到后端。后端出错时，经常要回看前面阶段产生的表示是否正确。

## 后端闭环：Spill 后为什么要回到 Liveness

寄存器分配如果发现 actual spill，会重写程序：

```text
use spilled t -> load t_new, [slot_t]
def spilled t -> store t_new, [slot_t]
```

这会新增指令和 temporary，改变 CFG 的 `use/def`、`in/out` 和干涉图。因此流程不是：
...
```text
RegAlloc -> actual spill -> Rewrite -> Flowgraph -> Liveness -> Interference -> RegAlloc
```

考试问“为什么 spill 后要重新分析”，答案就是：程序已经变了，旧的活跃变量和干涉图不再适用。
