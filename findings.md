...
- 在线答案抽屉出现题号全部显示为 `1.` 的根因在阅读器有序列表渲染：`markdown_to_html()` 识别 `^\s*\d+\.\s+` 后丢弃了源 Markdown 的显式编号，每个被段落、表格或 SVG 切开的 `<ol>` 都从浏览器默认的 1 开始。修复方向是保留分段列表的首项编号，必要时输出 `<ol start="N">`。
- 用户点名的第 02 章第 2 题题干要求“对正则表达式 `(a|b)*abb` 构造一个 NFA，再用子集构造得到 DFA”，但当前答案直接给“按后缀前缀长度构造 DFA”，没有先给 NFA 状态/转移，也没有展示 NFA 子集构造表。
- 本轮审计范围：所有章节的 `## 练习` 段落与 `23_练习参考答案.md` 中相应 `## NN` 答案段落；重点检查题号数量、题意匹配、构造题过程是否完整、练习答案是否存在事实错误或过度跳步。
- 自动题号抽取发现：第 08 章答案第 3 题编号行为空；第 12 章练习 6 题但答案只有 4 条；第 13 章练习 6 题但答案只有 4 条。已在答案文件中拆分并补齐。
