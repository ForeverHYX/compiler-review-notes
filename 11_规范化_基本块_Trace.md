...
- 概念题：能解释 canonical form、linearize、basic block、trace、fall-through。
- 手算题：能消除 ESEQ，处理嵌套 CALL，能把线性语句切成 basic blocks。
- 调度题：能生成 trace covering，并处理 CJUMP 的 false fall-through、条件反转和插入假分支标签。
- 英文题：看到 `reorder`、`commute`、`marked block`、`condition negation` 能知道在哪个步骤使用。
