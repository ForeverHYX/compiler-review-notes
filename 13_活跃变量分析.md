...
- 概念题：能解释 live variable、live range、CFG、use/def、gen/kill、fixed point。
- 手算题：能列 use/def/succ，能按反向方程迭代 in/out，能处理分支和循环 CFG。
- 构图题：能从 live-out 构造 interference graph，并说明 move 指令的特殊处理。
- 英文题：看到 `flow graph`、`predecessor/successor`、`interference graph` 能写出对应表格或图。
